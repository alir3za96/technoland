...
export default function ProductSliderList({ title, bg_color, color, func, sliderState }) {
    // const listProductsAllShow = useSelector(state => state.listProductsAllShow)
    const num = [1, 2, 3, 4, 5, 6]
    const { products,favorite_products, loading, error } = sliderState
                    ...
                    {loading ?
                        ...
                        : (
                            <>
                                {favorite_products?(
                                    <>
                                    {favorite_products.map((product, index) => (
                                        ...
                                        <div className="product-box" >
                                            ...
                                            <div className="product-box-image-container">
                                                <div className="product-box-image">
                                                    <Link to={`/products/${product.product._id}`} onClick={()=>window.scroll(0,0)}>
                                            ...
                                            <div className="product-box-list-body-container pb-3">
                                                <Link to={`/products/${product.product._id}`} onClick={()=>window.scroll(0,0)}>
                                                    ...
                                                    <div className="product-box-price">
                                                        <span className="product-price-current">{Intl.NumberFormat('fa-IR').format(product.product.price)}<span className='price-toman'>تومان</span></span>
                                                    </div>
                                                </Link>
